...
def InvKinematics(frame, path_line, goal_mark, hSoft, hSkel, hForce,
    ...
    f_ext_fun = lambda t: np.array([0, -0.5, -0])
    ...
    softPts, skelPts, tip_coord = kin.Compute_Soft_Curve(q_next)
    ...
    try:
        ...
        rate_fb = 0.15 * (1.0  + max(0.0, 2.0 - error_norm))
        rate_ff = 5e-6
    ...
    u_ells = np.round(np.clip(u_ells, 0, 50), 3)
    ...
    hForce.set_data([tip_coord[0], tip_coord[0] + 3*f_vec[0]], [tip_coord[1], tip_coord[1] + 3*f_vec[1]])
    hForce.set_3d_properties([tip_coord[2], tip_coord[2] + 3*f_vec[2]])
    ...
    return path_line, goal_mark, hSoft, hSkel, hForce, time_text, len1_mark, len2_mark, len3_mark
...
def main():
    ...
    PathCoords = np.array([[ 0], [0.3], [28]])

    u_ells = np.array([5, 5  , 5])

    q0 = kin.q_no_load(u_ells)

    _,_,tip_prev = kin.Compute_Soft_Curve(q0)

    # Physical Impedance of the system, the stiffness and the D matrix
    K = np.diag([15,15,0.5,15,15,0.5,15,15,0.5])
    D = np.diag([0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5])
    ...
    hForce, = ax1.plot([], [], [], '-', color='g', linewidth=3, label='Force Vector')
    ...
    anim = animation.FuncAnimation(
        fig, InvKinematics, fargs = (path_line, goal_mark, hSoft, hSkel, hForce,
                                     time_text, len1_mark, len2_mark, len3_mark, kin, PathCoords, K,D),
