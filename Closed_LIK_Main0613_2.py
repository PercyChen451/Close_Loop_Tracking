...
def update_control_loop(tipCoords, kin, PathCoords, arduino, dt):
    ...
    K = np.diag([15*scale, 15*scale, 0.5*scale, 15*scale, 15*scale, 0.5*scale, 15*scale, 15*scale, 0.5*scale])
...
def main():
    ...
    pathCoords = kin2.generate_circle(10, 27, 20)
        ...
        if red_pos_yz and red_pos_xz:
         ...
         delta_u, volumes, pressures, u_target, new_vol) = update_control_loop(
            ...
            data = {
                ...
                'q_current' : q_next.tolist() if q_next is not None else None
