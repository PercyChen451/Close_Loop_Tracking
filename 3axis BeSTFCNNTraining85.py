...
def prepare_data(filepath, n_lags=5, test_size=0.15, val_size=0.15, shift_i=2):
    ...
    # Feature engineering (updated for B3 values)
    ...
    # Prepare normalization parameters dictionary (updated for B3 features)
    norm_params = {
        'X_median': X_median,
        'X_iqr': X_iqr,
        'Y_median': Y_median,
...
# Main Execution
def main():
    # Prepare data with new B3 values
    (X_train, Y_train, X_val, Y_val, 
     X_test, Y_test, norm_params) = prepare_data('your_data.csv', n_lags=3)
    ...
    # Create loaders (adjust batch sizes if needed)
    train_loader = DataLoader(TensorDataset(X_train, Y_train), batch_size=512, shuffle=True)
    val_loader = DataLoader(TensorDataset(X_val, Y_val), batch_size=256)
    test_loader = DataLoader(TensorDataset(X_test, Y_test), batch_size=256)
    
    # Initialize model (automatically adjusts to input size)
    model = ForceCalibrationModel(X_train.shape[1])
    print(f"\nModel initialized with input size: {X_train.shape[1]}")
    ...
    print("\nSaved TorchScript model for deployment")
